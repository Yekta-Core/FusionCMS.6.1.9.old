...
class Template
{
	...
	public function __construct()
	{
		...
		$this->style_path = base_url()."application/themes/".$this->theme."/css/";
		$this->image_path = base_url()."application/themes/".$this->theme."/images/";
	...
	private function handleNormalPage($content, $css, $js)
	{
		...
		return $output = $this->CI->smartyengine->view($this->theme_path."template.tpl", $theme_data, true);
	}
	...
	private function getModals()
	{
		...
		// Load the modals
		$modals = $this->CI->smartyengine->view($this->theme_path."views/modals.tpl", $modal_data, true);
	...
	private function getHeader($css = "", $js = "")
	{
		...
		// Load the theme
		return $this->CI->smartyengine->view($this->view_path."header.tpl", $header_data, true);
	}
	...
	public function loadPage($page, $data = array())
	{
		...
		if($data['module'] == "default")
		{
			...
			return $this->CI->smartyengine->view($this->theme_path . $page, $data, true, true);
		}
		...
		{
			return $this->CI->smartyengine->view($themeView, $data, true);
		}

		return $this->CI->smartyengine->view('modules/'.$data['module'].'/views/'.$page, $data, true);
	}
